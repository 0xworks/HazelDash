...
PlayerController::PlayerController(Hazel::Entity entity, HazelDashLayer* level, float fixedTimeStep, float pushProbability)
: NativeScript{entity}
, m_Level{level}
, m_FixedTimeStep{fixedTimeStep}
, m_PushProbability{pushProbability}
{}
...
void PlayerController::OnUpdate(Hazel::Timestep ts) {
	...
	if (m_FixedUpdateAccumulatedTs > m_FixedTimeStep) {
		m_FixedUpdateAccumulatedTs -= m_FixedTimeStep;
		...
		if ((row != oldRow) || (col != oldCol)) {
			Hazel::Entity entityAtNewPos = m_Level->GetEntity(row, col);
			auto tile = entityAtNewPos.GetComponent<Tile>();
			m_Level->SwapEntities(oldRow, oldCol, row, col);
			m_Level->ClearEntity(oldRow, oldCol);
			if (IsDoor(tile)) {
				m_Level->OnLevelCompleted();
			}
			if (IsCollectable(tile)) {
				m_Level->OnIncreaseScore();
			}
		}
	}
...
bool PlayerController::IsIdle(PlayerState state) {
	return (state == PlayerState::Idle) || (state == PlayerState::Blink) || (state == PlayerState::FootTap) || (state == PlayerState::BlinkFootTap);
}


PlayerState PlayerController::SetPlayerBlinkState(PlayerState state) {
	switch (state) {
...
PlayerState PlayerController::ClearPlayerBlinkState(PlayerState state) {
	switch (state) {
		case PlayerState::Idle:
...
PlayerState PlayerController::SwapPlayerFootTapState(PlayerState state) {
	switch (state) {
		case PlayerState::Idle:
			state = PlayerState::FootTap;
			break;
...
const Animation& PlayerController::GetPlayerAnimation(PlayerState state, bool lastWasLeft) {
...
bool PlayerController::TryMovePlayer(Hazel::TransformComponent& transformComponent, const std::pair<int, int> direction, const bool ctrlPressed) {
	bool retVal = false;
	int row = static_cast<int>(transformComponent.Translation.y);
	int col = static_cast<int>(transformComponent.Translation.x);
	Hazel::Entity entity = m_Level->GetEntity(row + direction.first, col + direction.second);
	...
	if (CanBeOccupied(tile)) {
		...
		if (Random::Uniform0_1() < m_PushProbability) {
			...
			Hazel::Entity entityBelow = m_Level->GetEntity(rowBelow, colBelow);
			Hazel::Entity entityAcross = m_Level->GetEntity(rowAcross, colAcross);
			...
			if (!IsEmpty(tileBelow) && IsEmpty(tileAcross)) {
				m_Level->SwapEntities(rowAcross, colAcross, row + direction.first, col + direction.second);
